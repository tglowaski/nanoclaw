...
function createSchema(database: Database.Database): void {
  ...
  try {
    ...
    database.exec(
      `UPDATE chats SET channel = 'imessage', is_group = 0 WHERE jid LIKE 'imsg:%' AND jid NOT LIKE 'imsg-group:%'`,
    );
    database.exec(
      `UPDATE chats SET channel = 'imessage', is_group = 1 WHERE jid LIKE 'imsg-group:%'`,
    );
  } catch {
...
/** Returns the active database handle (must be called after initDatabase). */
